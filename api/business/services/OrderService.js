...
class OrderService {
  ...
  async createOrder(orderData) {
    try {
      ...
      // Determine order type for ID generation
      const orderType = OrderIdGenerator.determineOrderType({
        items: processedItems,
        isCustom: false // This is a regular order, not custom
      });
      ...
      try {
        ...
        // 9. Trigger the onEnter action for the initial 'pending' state
        // Note: We don't call transitionState because the order is already in 'pending' state by default
        // Instead, we manually trigger the onEnter action for the pending state
        if (order.status === 'pending') {
          const workflow = WorkflowManager.workflows.order;
          const pendingStateConfig = workflow.states['pending'];
          if (pendingStateConfig.actions && pendingStateConfig.actions.onEnter) {
            await pendingStateConfig.actions.onEnter(order, {
              action: 'orderCreated',
              user: orderData.createdBy || 'customer'
            });
          }
        } else {
          // If for some reason the order is not in pending state, do a proper transition
          await WorkflowManager.transitionState(order, 'order', order.status, {
            action: 'orderCreated',
            user: orderData.createdBy || 'customer'
          });
        }
  ...
  async createCustomOrder(customOrderData) {
    try {
      ...
      // 5. Trigger the onEnter action for the initial 'pending' state
      // Note: We don't call transitionState because the order is already in 'pending' state by default
      // Instead, we manually trigger the onEnter action for the pending state
      const workflow = WorkflowManager.workflows.customOrder;
      const pendingStateConfig = workflow.states['pending'];
      if (pendingStateConfig.actions && pendingStateConfig.actions.onEnter) {
        await pendingStateConfig.actions.onEnter(customOrder, {
